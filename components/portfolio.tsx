...
// ─────────────────────────────────────────────────────────────────────────
// Portfolio images are served from /public/portfolio/ (committed, versioned,
// no external dependency). Drop a replacement photo at the same filename to
// swap real client photography in later — no code change needed.
// ─────────────────────────────────────────────────────────────────────────
const img = (id: string) => `/portfolio/${id}.jpg`;
...
const portfolioItems: PortfolioItem[] = [
  ...
  },
  {
    ...
    category: 'chandelier-installation',
    location: 'Prosper, TX',
    image: img('1556909114-44e3e70034e2'),
  ...
  },
  {
    ...
    image: img('1600607686527-6fb886090705'),
  ...
  // Outlet & Circuit Projects (6 items)
  {
    ...
    location: 'Southlake, TX',
    image: img('1513584684374-8bab748fbf90'),
  ...
  },
  {
    ...
    category: 'outlet-circuit',
    location: 'Celina, TX',
    image: img('1560185893-a55cbc8c57e8'),
...
/**
 * next/image with a graceful fallback: if the local asset is ever missing or
 * fails to decode (e.g. a renamed/removed file), we swap in a neutral icon
 * slab so the layout never breaks.
 */
function PortfolioImage({ src, alt, sizes, className }: { src: string; alt: string; sizes: string; className?: string }) {
  const [errored, setErrored] = useState(false);

  if (errored) {
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-[var(--color-surface-900)]">
        <ImageIcon className="h-10 w-10 text-[var(--color-text-muted)]" aria-hidden />
      </div>
    );
  }
